...
@author: Hossein Davarzanisani
Student No=300102825
"""
...
#it is a function to handle the model when it has zero or one dimensions in some defined functions
def getdimension(d):
    if d==0:
        return 1
    elif d==1:
...
d = int(input("Enter the model dimension(integer): "))
...
init_theta = np.random.randn(getdimension(d),1)
    ...
    #taking all 441 possible combination of N,d,var
     ...
     for i in range(0,len(all_comb)):
         ...
         while(M<50):
            ...
            theta,X_b=gradient_descent(x_train,y_train, d, lr, 100,np.random.randn(getdimension(d),1))
         ...
         theta_final=theta_all.mean(1).reshape(getdimension(d),1)
         
         Ein_avg=Ein_all.mean(0).item()
         print("for N= ",N,"      dimension= ",d, "     variance= ",var)
...
for i in [2,10,200]:
    ...
    plt.title("Change in bias vs dimension for fixed variance(var=0.1)",fontsize=12)
plt.legend()
plt.show()
...
for i in [10,50,100]:#,200]:
    ...
    plt.title("change in Ebias vs variance for fixed dimension(dim=8)",fontsize=12)
plt.legend()
plt.show()
###################################################
